...
<style>
    ...
    .ql-editor { min-height: 400px; padding: 1.5rem; line-height: 1.8; color: #4A5568; font-size: 15px; resize: vertical; overflow: auto; }
    .ql-editor h2 { font-size: 1.6rem; font-weight: 700; color: #0B1528; margin-top: 2rem; margin-bottom: 1rem; }
    .ql-editor h3 { font-size: 1.2rem; font-weight: 700; color: #0B1528; margin-top: 1.5rem; margin-bottom: 0.5rem; }
    ...
    /* Image toolbar */
    .img-toolbar { display: none; position: absolute; background: #1e293b; border-radius: 0.5rem; padding: 4px; gap: 4px; z-index: 100; flex-wrap: wrap; box-shadow: 0 4px 16px rgba(0,0,0,0.25); }
    .img-toolbar.active { display: flex; }
    .img-toolbar button { background: transparent; border: none; color: #e2e8f0; cursor: pointer; padding: 4px 8px; border-radius: 4px; font-size: 11px; font-weight: 600; white-space: nowrap; }
    .img-toolbar button:hover { background: rgba(255,255,255,0.1); }
    .img-toolbar input[type=range] { width: 90px; accent-color: #82E682; }
    .ql-upload-images { display: flex !important; align-items: center; gap: 4px; }
    /* Resize handle on selected img */
    .ql-editor img.img-selected { outline: 2px solid #82E682; outline-offset: 2px; }
    /* Resizable editor container */
    .blog-editor-wrapper { resize: vertical; overflow: hidden; min-height: 450px; border-radius: 0 0 0.75rem 0.75rem; }
</style>
<script>
document.addEventListener('alpine:init', () => {
    if (window.__blogEditorRegistered) return;
    ...
    Alpine.data('blogEditor', () => ({
        ...
        init() {
            ...
            this.$nextTick(() => {
                const container = this.$refs.blogEditorContainer;
                ...
                wire.$watch('content', (val) => {
                    if (val !== undefined && val !== this.quill.root.innerHTML) {
        ...
        _setupImageInteractions() {
            ...
            tb.innerHTML = `
                <button data-action="float-left" title="Flotter à gauche">← Gauche</button>
                <button data-action="float-right" title="Flotter à droite">Droite →</button>
                <button data-action="full-width" title="Pleine largeur">↔ Plein</button>
                <button data-action="normal" title="Normal">Normal</button>
                <span style="color:#94a3b8;padding:2px 4px;font-size:11px">Taille:</span>
                <input type="range" min="10" max="100" value="100" data-action="resize" title="Redimensionner">
                <button data-action="replace" title="Remplacer">🔄 Remplacer</button>
                <button data-action="remove" title="Supprimer" style="color:#f87171">✕</button>
            `;
            ...
            editorRoot.addEventListener('click', (e) => {
                if (e.target.tagName === 'IMG') {
                    this._selectImage(e.target, tb);
                } else {
                    this._deselectImage(tb);
                }
            });
            ...
            tb.addEventListener('mousedown', (e) => {
                ...
                if (action === 'float-left') { img.className = ''; img.classList.add('img-float-left','img-selected'); }
                else if (action === 'float-right') { img.className = ''; img.classList.add('img-float-right','img-selected'); }
                else if (action === 'full-width') { img.className = ''; img.classList.add('img-full','img-selected'); img.style.width = ''; }
                ...
                this.quill.update();
                wire => this.$wire.set('content', this.quill.root.innerHTML);
            });

            tb.addEventListener('input', (e) => {
                if (e.target.dataset.action === 'resize' && this._selectedImg) {
                    this._selectedImg.style.width = e.target.value + '%';
                    this._selectedImg.style.maxWidth = e.target.value + '%';
                    this._repositionToolbar(this._selectedImg, tb);
                }
            });

            document.addEventListener('scroll', () => {
                if (this._selectedImg) this._repositionToolbar(this._selectedImg, tb);
            }, true);
        },
        ...
        destroy() {
            if (this._imgToolbar) this._imgToolbar.remove();
            this.quill = null;
        },
    }));
});
...
<div class="p-6 sm:p-8 bg-[#FAFAFC] min-h-screen">
    ...
    <div class="mb-8">
        ...
        <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
            <div>
                <span class="inline-flex rounded-full bg-[#82E682] px-4 py-1.5 text-[0.7rem] font-extrabold uppercase tracking-widest text-[#0a471b] mb-3">
    ...
    <div class="grid gap-6 lg:grid-cols-3">
        ...
        <div class="space-y-6 lg:col-span-2">
            ...
            <div class="rounded-2xl border border-slate-200/80 bg-white p-6 shadow-sm">
                <label class="block text-[0.65rem] font-extrabold uppercase tracking-widest text-[#23803B] mb-3">Titre de l'article</label>
                <input type="text" wire:model="title" placeholder="Saisissez un titre accrocheur…"
            ...
            {{-- Excerpt --}}
            <div class="rounded-2xl border border-slate-200/80 bg-white p-6 shadow-sm">
                <label class="block text-[0.65rem] font-extrabold uppercase tracking-widest text-[#23803B] mb-3">Résumé</label>
                <textarea wire:model="excerpt" rows="3" placeholder="Rédigez une accroche…"
                          class="w-full rounded-xl border border-slate-100 bg-[#FAFAFC] px-4 py-3 text-sm text-slate-600 placeholder:text-slate-300 outline-none focus:border-[#0B1528] focus:bg-white focus:ring-2 focus:ring-[#0B1528]/10 resize-none transition"></textarea>
            ...
            <div class="overflow-hidden rounded-2xl border border-slate-200/80 bg-white shadow-sm">
                <div class="border-b border-slate-100 px-6 py-4 flex items-center justify-between gap-3">
                    <div class="flex items-center gap-3">
                        <span class="size-2.5 rounded-full bg-[#82E682]"></span>
                        <span class="text-[0.65rem] font-extrabold uppercase tracking-widest text-[#23803B]">Contenu de l'article</span>
                    </div>
                    <span class="text-[0.6rem] text-slate-400 font-medium">Cliquez sur une image pour la redimensionner · Glissez le bas pour agrandir l'éditeur</span>
                </div>
                <div x-data="blogEditor" wire:ignore class="blog-editor-wrapper">
                    <div x-ref="blogEditorContainer"></div>
                </div>
        ...
        <div class="space-y-5">
            ...
            <div class="rounded-2xl border border-slate-200/80 bg-white p-5 shadow-sm">
                ...
                @if($newImage)
                <div class="mb-4 overflow-hidden rounded-xl h-36 w-full">
                    <img src="{{ $newImage->temporaryUrl() }}" class="w-full h-full object-cover" alt="">
                </div>
                @endif
                <label class="flex cursor-pointer flex-col items-center justify-center rounded-xl border-2 border-dashed border-slate-200 bg-[#FAFAFC] py-6 text-center hover:border-[#82E682] hover:bg-[#f0fbf0] transition group">
                    <flux:icon.photo class="size-8 text-slate-300 group-hover:text-[#23803B] mb-2 transition" />
                    <p class="text-sm font-bold text-slate-500 group-hover:text-[#23803B]">Choisir une image</p>
            ...
            <div class="rounded-2xl border border-slate-200/80 bg-white p-5 shadow-sm">
                ...
                <div class="grid grid-cols-2 gap-3">
                    <button type="button" wire:click="$set('is_published', false)"
                            class="flex flex-col items-center gap-2 rounded-xl border-2 px-3 py-4 transition text-center {{ !$is_published ? 'border-[#0B1528] bg-[#0B1528] text-white' : 'border-slate-200 bg-slate-50 text-slate-500 hover:border-slate-300' }}">
                        <flux:icon.pencil-square class="size-5" />
                        <span class="text-xs font-extrabold uppercase">Brouillon</span>
                    </button>
                    <button type="button" wire:click="$set('is_published', true)"
                            class="flex flex-col items-center gap-2 rounded-xl border-2 px-3 py-4 transition text-center {{ $is_published ? 'border-[#82E682] bg-[#82E682] text-[#0a471b]' : 'border-slate-200 bg-slate-50 text-slate-500 hover:border-[#82E682]/50' }}">
            ...
            <div class="rounded-2xl border border-slate-200/80 bg-white p-5 shadow-sm">
                ...
                <select wire:model="category" class="w-full rounded-xl border border-slate-100 bg-[#FAFAFC] px-4 py-3 text-sm font-semibold text-[#0B1528] outline-none focus:border-[#0B1528] focus:ring-2 focus:ring-[#0B1528]/10 transition">
            ...
            <div class="rounded-2xl border border-slate-200/80 bg-white p-5 shadow-sm">
                ...
                <input type="text" wire:model="tags_input" placeholder="architecture, urbanisme…"
...
<div class="p-6 sm:p-8">
    ...
    <div class="flex flex-col gap-5 sm:flex-row sm:items-start sm:justify-between mb-8">
        <div>
            ...
            <h2 class="mt-2 text-3xl font-black tracking-tight text-slate-900">Blog & Articles</h2>
            <p class="mt-2 text-sm text-slate-600">Rédigez, publiez et gérez les articles du blog.</p>
        </div>
    ...
    <div class="grid grid-cols-3 gap-4 mb-8">
        <div class="rounded-2xl border border-slate-100 bg-white p-5 shadow-sm">
            <div class="flex h-10 w-10 items-center justify-center rounded-xl bg-blue-50 mb-3">
                <flux:icon.newspaper class="size-5 text-blue-600" />
            </div>
        ...
        <div class="rounded-2xl border border-slate-100 bg-white p-5 shadow-sm">
            ...
            <p class="text-2xl font-black text-slate-900">{{ $stats['published'] }}</p>
        ...
        <div class="rounded-2xl border border-slate-100 bg-white p-5 shadow-sm">
            ...
            <p class="text-2xl font-black text-slate-900">{{ $stats['draft'] }}</p>
    ...
    <div class="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-slate-200">
                <thead class="bg-[#04103A] text-white">
                    <tr>
                        <th class="px-6 py-4 text-left text-xs font-semibold uppercase tracking-wide">Titre</th>
                        <th class="px-6 py-4 text-left text-xs font-semibold uppercase tracking-wide">Catégorie</th>
                        <th class="px-6 py-4 text-left text-xs font-semibold uppercase tracking-wide">Date</th>
                        <th class="px-6 py-4 text-left text-xs font-semibold uppercase tracking-wide">Statut</th>
                ...
                <tbody class="divide-y divide-slate-100">
                    ...
                    <tr class="transition hover:bg-slate-50">
                        <td class="px-6 py-4">
                            <div class="flex items-center gap-3">
                                ...
                                <div class="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-lg bg-indigo-50">
                                    <flux:icon.newspaper class="size-5 text-indigo-300" />
                                </div>
                                @endif
                                <span class="max-w-xs truncate text-sm font-semibold text-[#04103A]">{{ $post->title }}</span>
                            </div>
                        </td>
                        <td class="px-6 py-4">
                            @if($post->category)
                        ...
                        <td class="px-6 py-4">
                            <div class="flex items-center justify-center gap-2">
                                <button type="button" wire:click="openEdit({{ $post->id }})"
                                ...
                                <button type="button" wire:click="togglePublish({{ $post->id }})"
                                    ...
                                    <flux:icon.{{ $post->is_published ? 'eye-slash' : 'eye' }} class="size-4" />
                                </button>
                    ...
                    <tr>
                        <td colspan="5" class="px-6 py-14 text-center">
                            <flux:icon.newspaper class="size-10 text-slate-200 mx-auto mb-3" />
                            <p class="text-sm font-semibold text-slate-500">Aucun article trouvé.</p>
                            <p class="text-xs text-slate-400 mt-1">Créez votre premier article.</p>
                        </td>
                    </tr>
                    @endforelse
                </tbody>
        ...
        <div class="flex flex-col gap-4 border-t border-slate-200 bg-white px-6 py-4 sm:flex-row sm:items-center sm:justify-between">
            <p class="text-sm text-slate-500">
                {{ $posts->firstItem() ?? 0 }}–{{ $posts->lastItem() ?? 0 }} sur {{ $posts->total() }} articles
            </p>
            <div>{{ $posts->links() }}</div>
        </div>
    </div>
